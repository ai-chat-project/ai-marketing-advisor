...
if (!STRIPE_KEY) {
  export default function handler(req, res) {
    return res.status(500).send('Missing STRIPE_SECRET_KEY in environment.');
  };
}
...
async function ensureProduct(){
  ...
  const found = list.data.find(p => p.metadata?.slug===PRODUCT_SLUG) || list.data.find(p => p.name===PRODUCT_NAME);
...
async function ensurePrice(productId, plan){
  ...
  const existing = list.data.find(p => p.lookup_key===LOOKUP[plan] && p.recurring?.interval==='month' && p.currency===CURRENCY);
...
export default async function handler(req, res){
  try{
    ...
    const { plan } = req.body || {};
    if (!['starter','pro','premium'].includes(plan)) return res.status(400).send('Unknown plan');
    ...
    if (plan === 'pro') payload.subscription_data = { trial_period_days: 7 };

    const session = await stripe.checkout.sessions.create(payload);
    return res.status(200).json({ url: session.url });
  }catch(e){
    console.error('create-checkout error:', e);
    return res.status(500).send(e?.message || 'Stripe create-checkout failed');
  }
}
